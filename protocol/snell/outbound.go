...
// unconditionally.
const quicDestCacheTTL = 10 * time.Minute
...
type Outbound struct {
	...
	psk        []byte
	version    int
	// quicDestCache records destinations that recently used the QUIC proxy path,
	// keyed by M.Socksaddr. Values are time.Time (last seen).
	quicDestCache sync.Map
}

func (h *Outbound) isRecentQUICDest(destination M.Socksaddr) bool {
	v, ok := h.quicDestCache.Load(destination)
	if !ok {
		return false
	}
	if time.Since(v.(time.Time)) > quicDestCacheTTL {
		h.quicDestCache.Delete(destination)
		return false
	}
	return true
}

func (h *Outbound) markQUICDest(destination M.Socksaddr) {
	h.quicDestCache.Store(destination, time.Now())
}
...
func (h *Outbound) ListenPacket(ctx context.Context, destination M.Socksaddr) (net.PacketConn, error) {
	...
	if h.version >= snell.Version5 {
		sniffQUIC := metadata.Protocol == C.ProtocolQUIC || h.isRecentQUICDest(destination)
		return newV5LazyPacketConn(ctx, h, destination, sniffQUIC), nil
	}
...
func (c *v5LazyPacketConn) initConn(p []byte, addr net.Addr) {
	...
	if useQUIC {
		...
		if err == nil {
			c.outbound.markQUICDest(c.destination)
		}
	} else {
