...
func TestClientLazyCachePreservesClientSubnetOPTHeader(t *testing.T) {
	t.Parallel()

	client := NewClient(ClientOptions{LazyCacheTTL: 3600})
	transport := newClientSubnetCacheTestTransport()
	clientSubnet := netip.MustParsePrefix("1.1.1.0/24")
	question := newClientSubnetCacheQuery().Question[0]
	cacheKey := dnsCacheKey{Question: question, clientSubnet: clientSubnet}
	staleResponse := SetClientSubnet(clientSubnetCacheResponse(newClientSubnetCacheQuery(), netip.MustParseAddr("198.51.100.1")), clientSubnet)
	staleResponse.IsEdns0().SetUDPSize(1232)
	staleResponse.IsEdns0().SetDo()
	originalOPTTTL := staleResponse.IsEdns0().Hdr.Ttl
	client.cache.AddWithLifetime(cacheKey, &dnsMsg{msg: staleResponse, expireTime: time.Now().Add(-time.Second), lazyCache: true}, time.Hour)

	response, _, stale := client.loadResponse(cacheKey, transport)
	require.True(t, stale)
	require.Equal(t, originalOPTTTL, response.IsEdns0().Hdr.Ttl)
}
